...
<template>
    <div v-if="loaded" v-for="dentist in dentists" class="mt-5 w-full px-5 py-2 flex justify-between hover:shadow-md duration-200 border border-slate-500 cursor-pointer">
        <div class="font-medium">{{ dentist.name }}</div>
        <div class="price">{{ dentist.position }} руб.</div>
    </div>
</template>
